...
object DSL {
  ...
  def VL(labels: Label*) = labels.map(Vertex(_)).toArray
  def VL(labels: Range) = labels.map(Vertex(_)).toArray
  ...
  def C(labels: Label*) = labels.map(Vertex(_)).toArray // connectors
  ...
  def nt(l: Label, c: Product = None) = {
    ...
    NonTerminal(l, c.productIterator.toList.asInstanceOf[List[Int]].map(Vertex(_)).toArray)
  }
}
