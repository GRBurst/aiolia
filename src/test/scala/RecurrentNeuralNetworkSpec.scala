...
class RecurrentNeuralNetworkSpec extends org.specs2.mutable.Specification {
  "neuronal network" >> {
    "labels of vertices must be in order 0..|vertices|" >> {
      RecurrentNeuralNetwork(in = Nil, out = Nil, Graph(V(1, 2), E(1 -> 2))) must throwAn[AssertionError]
    }

    "simple" >> {
      val n = RecurrentNeuralNetwork(in = VL(0, 1), out = VL(2), Graph(V(0 to 2), E(0 -> 2, 1 -> 2), vData(2 -> -1), eData((0 -> 2) -> 0.4f, (1 -> 2) -> 0.6f)))
    ...
    "two steps" >> {
      val n = RecurrentNeuralNetwork(in = VL(0, 1), out = VL(3), Graph(V(0 to 3), E(0 -> 2, 1 -> 2, 0 -> 3, 2 -> 3), vData(2 -> -1, 3 -> 2), eData((0 -> 2) -> 0.4f, (1 -> 2) -> 0.6f, (0 -> 3) -> 0.1f, (2 -> 3) -> 0.5f)))
      n.setInputData(List(2, 4))
      n.think()
      n.think()
    ...
    "fail on too much input data" >> {
      val n = RecurrentNeuralNetwork(in = VL(0), out = VL(1), Graph(V(0 to 1), E(0 -> 1)))
