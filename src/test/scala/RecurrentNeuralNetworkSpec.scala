...
class RecurrentNeuralNetworkSpec extends org.specs2.mutable.Specification {
  "neuronal network" >> {
    ...
    "two steps" >> {
      val n = Recurrent(in = VL(0, 1), out = VL(3), Graph(V(0 to 3), E(0 -> 2, 1 -> 2, 0 -> 3, 2 -> 3), vData(2 -> -1, 3 -> 2), eData((0 -> 2) -> 0.4f, (1 -> 2) -> 0.6f, (0 -> 3) -> 0.1f, (2 -> 3) -> 0.5f)))
      n.setInputData(List(2, 4))
      n.think()
      n.think()
    ...
    "fail on too much input data" >> {
      val n = Recurrent(in = VL(0), out = VL(1), Graph(V(0 to 1), E(0 -> 1)))
