...
import aiolia.neuralNetwork.FeedForward
...
class FeedForwardNeuralNetworkSpec extends org.specs2.mutable.Specification {
  ...
  "input and output not connected" >> {
    val n = FeedForward(VL(0), VL(1), graph(V(0, 1), E(), vData(0 -> 0.8, 1 -> 2.1)))
    n.compute(Array(0.2)) mustEqual Array(sigmoid(2.1))
    n.compile()
    n.compute(Array(0.2)) mustEqual Array(sigmoid(2.1))
  }
  "one input -> one output" >> {
    val n = FeedForward(VL(0), VL(1), graph(V(0, 1), E(0 -> 1),
      vData(0 -> 0.1, 1 -> 0.2),
      eData((0 -> 1) -> -0.7)))
  ...
  "two inputs -> one output" >> {
    val n = FeedForward(VL(0, 1), VL(2), graph(V(0, 1, 2), E(0 -> 2, 1 -> 2),
  ...
  "skipping one layer" >> {
    ...
    val n = FeedForward(VL(0, 1), VL(3), graph(V(0, 1, 2, 3), E(0 -> 2, 1 -> 2, 2 -> 3, 0 -> 3),
  ...
  "multiple inputs and outputs" >> {
    val n = FeedForward(VL(0, 1), VL(3, 4), graph(V(0, 1, 2, 3, 4), E(0 -> 2, 1 -> 2, 2 -> 3, 2 -> 4),
